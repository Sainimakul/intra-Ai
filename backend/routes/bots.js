...
router.get('/:botId', authenticate, async (req, res) => {
  ...
  // Check if this bot is paused due to plan downgrade
  ...
  // Bot is "over limit" if it is NOT among the top N most recent bots
...
router.get('/public/:botId', async (req, res) => {
  const result = await pool.query(
...
router.post('/', authenticate, requireVerified, [
  ...
  if (parseInt(countResult.rows[0].count) >= maxBots) {
    return res.status(403).json({ error: `Your plan allows a maximum of ${maxBots} bot(s). Please upgrade.` });
  }
...
// ─── Update Bot ───────────────────────────────────────────────────────────────
router.put('/:botId', authenticate, async (req, res) => {
  const existing = await pool.query('SELECT id FROM bots WHERE id = $1 AND user_id = $2', [req.params.botId, req.user.id]);
  if (!existing.rows[0]) return res.status(404).json({ error: 'Bot not found' });

  // Check if this bot is over the plan's bot limit
  ...
  // Block all edits on over-limit bots — user must upgrade first
  if (isOverLimit) {
  ...
  if (req.body.show_branding === false && !plan.allow_branding_removal) {
    req.body.show_branding = true;
  }
  if (req.body.collect_email === true && !plan.allow_analytics) {
    req.body.collect_email = false;
  }
  // Full customization (font, custom colors, avatar URL) — Business only
  if (plan.plan_slug !== 'business') {
    delete req.body.font_family;
    delete req.body.avatar_url;
    // Allow theme colors only (set via predefined themes), not arbitrary hex
    // We don't strip primary_color/secondary_color since themes are Pro+ anyway
  }
...
// ─── Delete Bot ───────────────────────────────────────────────────────────────
router.delete('/:botId', authenticate, async (req, res) => {
  const result = await pool.query(
...
// ─── Get Bot Embed Code ───────────────────────────────────────────────────────
router.get('/:botId/embed', authenticate, async (req, res) => {
  const result = await pool.query('SELECT id, name FROM bots WHERE id = $1 AND user_id = $2', [req.params.botId, req.user.id]);
  if (!result.rows[0]) return res.status(404).json({ error: 'Bot not found' });
