...
// ─── Full Messages for a Conversation ────────────────────────────────────────
...
const toggleResolveConversation = async (req, res) => {
  const check = await pool.query(
    `SELECT c.id FROM conversations c
     JOIN bots b ON c.bot_id = b.id
     WHERE c.id = $1 AND b.user_id = $2`,
  ...
  await pool.query(
    'UPDATE conversations SET is_resolved = NOT is_resolved WHERE id = $1',
    [req.params.conversationId]
  );
  res.json({ message: 'Updated' });
};

// ─── All Conversations Across All User Bots ───────────────────────────────────
const getAllConversations = async (req, res) => {
  ...
  if (botId)    { where += ` AND c.bot_id = $${idx}`;           values.push(botId);    idx++; }
  if (dateFrom) { where += ` AND c.started_at >= $${idx}`;      values.push(dateFrom); idx++; }
  if (dateTo)   { where += ` AND c.started_at <= $${idx}::date + INTERVAL '1 day'`; values.push(dateTo); idx++; }
